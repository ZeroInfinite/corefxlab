...
namespace System.Numerics
{
    ...
    internal class BoolArithmetic : ITensorArithmetic<bool>
    {
        ...
        public void And(DenseTensor<bool> left, DenseTensor<bool> right, DenseTensor<bool> result)
        {
            ...
            if  ((result.IsReveresedStride == left.IsReveresedStride) && (result.IsReveresedStride == right.IsReveresedStride))
            {
                for(int i = 0; i < result.Length; i++)
                {
            ...
            else
            {
                ...
                ref int resultIndex = ref RefUtilities.Ternary(result.IsReveresedStride, ref colMajorIndex,  ref rowMajorIndex);
                ref int op1Index = ref RefUtilities.Ternary(left.IsReveresedStride , ref colMajorIndex, ref rowMajorIndex);
                
                ref int op2Index = ref RefUtilities.Ternary(right.IsReveresedStride, ref colMajorIndex, ref rowMajorIndex );

                var rowMajorStrides = !result.IsReveresedStride ? result.strides :
                                      !left.IsReveresedStride ? left.strides : 
                                      right.strides;
                var columnMajorStrides = result.IsReveresedStride ? result.strides :
                                         left.IsReveresedStride ? left.strides : 
                                         right.strides;
        ...
        public void And(DenseTensor<bool> tensor, bool scalar, DenseTensor<bool> result)
        {
            ...
            if  (result.IsReveresedStride == tensor.IsReveresedStride)
            {
                for(int i = 0; i < result.Length; i++)
                {
            ...
            else
            {
                ...
                ref int resultIndex = ref RefUtilities.Ternary(result.IsReveresedStride, ref colMajorIndex,  ref rowMajorIndex);
                ref int op1Index = ref RefUtilities.Ternary(tensor.IsReveresedStride , ref colMajorIndex, ref rowMajorIndex);
