...
namespace System.Numerics
{
    public class DenseTensor<T> : Tensor<T>
    {
        ...
        public DenseTensor(bool reverseStride = false, params int[] dimensions) : base(dimensions, reverseStride)
        {
            backingArray = new T[Length];
        }

        public DenseTensor(T[] fromBackingArray, bool reverseStride = false, params int[] dimensions) : base(dimensions, reverseStride)
        {
        ...
        {
            return new DenseTensor<T>((T[])backingArray.Clone(), strides[0] == 1, dimensions);
        }

        public override Tensor<T> CloneEmpty()
        {
            return new DenseTensor<T>(strides[0] == 1, dimensions);
        }

        public override Tensor<TResult> CloneEmpty<TResult>()
        {
            return new DenseTensor<TResult>(strides[0] == 1, dimensions);
        }
        ...
        public override Tensor<T> Reshape(params int[] dimensions)
        {
            ...
            return new DenseTensor<T>(Buffer, IsReveresedStride, dimensions);
        }
    }
}
