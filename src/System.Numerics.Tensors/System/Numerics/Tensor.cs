...
namespace System.Numerics
{
    ...
    public abstract class Tensor<T> : IList, ICollection, IEnumerable, IStructuralComparable, IStructuralEquatable
    {
        ...
        public bool IsReveresedStride => strides.Length > 0 && strides[0] == 1;
        ...
        {
            var result = new DenseTensor<T>(columMajor, size, size);
        ...
        public static DenseTensor<T> CreateFromDiagonal(Tensor<T> diagonal)
        {
            return CreateFromDiagonal(diagonal, 0);
        }

        public static DenseTensor<T> CreateFromDiagonal(Tensor<T> diagonal, int offset)
        {
            if (diagonal.Rank < 1)
            {
        ...
        public DenseTensor<T> GetDiagonal()
        {
            return GetDiagonal(0);
        }

        public DenseTensor<T> GetDiagonal(int offset)
        {
            ...
            var diagonalTensor = new DenseTensor<T>(IsReveresedStride, dimensions: newTensorDimensions);
